...
const updateSonarData = (angle: number, newData: Uint8Array) => {
  ...
  for (let i = 0; i < newData.length; i += chunkSize) {
    const chunk = newData.subarray(i, Math.min(i + chunkSize, newData.length));
    for (let j = 0; j < chunk.length; j++) {
      const color = props.getColorFromPalette(chunk[j], props.colorPalette);
      const index = textureStart + (i + j) * 4;
